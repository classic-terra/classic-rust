...
fn main() {
    ...
    let prebuilt_lib_dir = manifest_dir.join("libclassictesttube").join("artifacts");

    let lib_name = "classictesttube";

    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());

    let header = if std::env::var("DOCS_RS").is_ok() {
        manifest_dir
            .join("libclassictesttube")
            .join("artifacts")
            .join("libclassictesttube.docrs.h")
    } else {
        out_dir.join(format!("lib{}.h", lib_name))
    };
    // rerun when go code is updated
    println!("cargo:rerun-if-changed=./libclassictesttube");
...
fn build_libclassictesttube(out: PathBuf) {
